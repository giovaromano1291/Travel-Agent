...
.pl-nav-logo{display:flex;align-items:center;gap:10px;cursor:pointer}
...
export default function PlannerPage(){
  ...
  const [duration,setDuration]=useState(""),[style,setStyle]=useState(""),[budget,setBudget]=useState(""),[departure,setDeparture]=useState("")
  const [fmt,setFmt]=useState("mps"),[mods,setMods]=useState(""),[wantsFood,setWantsFood]=useState(null),[inp,setInp]=useState("")
  const [aiPer,setAiPer]=useState(""),[aiPerLoad,setAiPerLoad]=useState(false)
  const [planText,setPlanText]=useState(""),[planLoad,setPlanLoad]=useState(false)
  const [revText,setRevText]=useState(""),[revLoad,setRevLoad]=useState(false)
  ...
  useEffect(()=>{if(draftLoad)draftLoadRef.current=true;if(!draftLoad&&draftLoadRef.current){draftLoadRef.current=false;if(draftRef.current)draftRef.current.scrollTop=0}},[draftLoad])
  ...
  function extractClusters(text){
    ...
    let sd=0;for(const c of out)if(c.days)sd+=c.days
  ...
  async function genHotelsRevised(){
    ...
    const starsQ=budget==="luxury"?"&stars=5":budget==="economico"?"&stars=2":"&stars=4",bases=[]
    for(const n of nb){
      let hotels=await fetchHotelsForCity(n.city,budget)
      if(!hotels)hotels=[{name:"Cerca hotel a "+n.city,stars:3,zone:"centro",price:"vedi Booking",why:n.note||"Hotel consigliato",pros:["prezzi aggiornati"],best:true,url:"https://www.booking.com/search.html?ss="+encodeURIComponent(n.city)+starsQ}]
      bases.push({city:n.city,days:n.days,note:n.note||"",hotels});setHotelBases(JSON.parse(JSON.stringify(bases)))
    }
  ...
  function HCard({h,bi,city}){
    ...
    return <div className={"hcard"+(isSel?" sel":"")} onClick={()=>{const ck=[...selKeys],cn=[...selNames];const ex=ck.findIndex(k=>k.startsWith(bi+"-"));if(ex>=0){ck[ex]=sk;cn[ex]=city+": "+nm}else{ck.push(sk);cn.push(city+": "+nm)}setSelKeys(ck);setSelNames(cn)}}>
      {bs&&<div style={{display:"inline-block",background:"#1a1400",border:".5px solid "+G,borderRadius:10,padding:"2px 10px",fontSize:10,color:G,marginBottom:6}}>{"\u2605"} Miglior rapporto</div>}
      <div style={{fontSize:15,color:GL,fontWeight:500,marginBottom:4}}>{nm} {starsStr(st)}</div>
      <div style={{display:"flex",flexWrap:"wrap",gap:6,marginBottom:6}}>{[zn,pr,budget].filter(Boolean).map(tt=><span key={tt} style={{background:"#1a1a1a",border:".5px solid #333",borderRadius:12,padding:"3px 10px",fontSize:11,color:"#aaa"}}>{tt}</span>)}</div>
  ...
  const lks=[
    ...
    {l:"TripAdvisor",s:"Ristoranti e attivita",h:"https://www.tripadvisor.it/Search?q="+encodeURIComponent(dest)},
  ...
  const draftClusters=(()=>{
    const text=draftText||"",lines=text.split("\n"),giorni=[];let cur=null
    for(const l0 of lines){const l=l0.trim();if(l.match(/^\*{0,2}giorno\s*\d+/i)){if(cur)giorni.push(cur);cur={title:l.replace(/\*\*/g,"").trim(),items:[]}}else if(cur&&(l[0]==="-"||l[0]==="*")&&l.length>2){const item=l.slice(1).trim().replace(/\*\*/g,"");const lo=item.toLowerCase();if(item.length>3&&lo.indexOf("trasport")<0&&lo.indexOf("pagament")<0&&lo.indexOf("budget")<0&&lo.indexOf("app utili")<0&&lo.indexOf("prenotazion")<0){let syn=item.replace(/\s*\([^)]*\)\s*$/,"").trim();const ci=syn.search(/\s[-\u2013]\s|:\s|,\s(per|con|dove|che|e\s)/i);if(ci>10)syn=syn.slice(0,ci);if(syn.length>55)syn=syn.slice(0,52).trim()+"...";if(cur.items.length<4)cur.items.push(syn)}}}
    if(cur)giorni.push(cur);return giorni
  })()
    ...
    <>
      ...
      <div className="planner-page">
        <nav className="pl-nav">
          <div className="pl-nav-logo" onClick={()=>navigate("/")}><div className="pl-nav-icon">{"\u2708\uFE0F"}</div><div className="pl-nav-name">Travel <span>AI</span> Agent</div></div>
          <div className="pl-nav-right"><button className="pl-btn-sm" onClick={()=>navigate("/dashboard")}>&larr; Dashboard</button></div>
        </nav>
        ...
        <div className="wrap">
          ...
          {step===1&&<div className="card">
            ...
            <div className="ir"><input className="inp" placeholder="Es. Italia, Giappone, Kenya..." value={inp} onChange={e=>setInp(e.target.value)} onKeyDown={e=>{if(e.key==="Enter"&&inp.trim())onDest()}}/><button className="go" onClick={()=>{if(inp.trim())onDest()}}>{"\u203a"}</button></div>
          </div>}
          ...
          {step===3&&<div className="card">
            ...
            {(travType==="Gruppo di amici"||travType==="Famiglia"||travType==="Gruppo con bambini")&&<div className="sep"><div className="slbl">Adulti</div><div className="nr"><button className="nb" onClick={()=>setAdults(Math.max(1,adults-1))}>&#8722;</button><span className="nv">{adults}</span><button className="nb" onClick={()=>setAdults(adults+1)}>+</button></div></div>}
          ...
          {step===10&&<div className="card" style={{maxWidth:720}}>
            ...
            {hotelBases.length>0&&<div>
              <div style={{background:"#111",border:".5px solid "+BRD,borderRadius:12,padding:"14px 18px",marginBottom:"1.2rem"}}>
                <div style={{fontSize:11,letterSpacing:"2px",color:G,textTransform:"uppercase",fontWeight:600,marginBottom:"0.9rem"}}>{hotelBases.length>1?"\uD83D\uDDFA\uFE0F Soggiorno in "+hotelBases.length+" basi":"\uD83C\uDFE8 Alloggio unico"}</div>
                <div style={{display:"flex",flexWrap:"wrap",alignItems:"center",gap:6}}>
                  {hotelBases.map((b,bi)=>(
                    <React.Fragment key={bi}>
                      <div style={{background:"#1a1400",border:".5px solid "+G,borderRadius:20,padding:"6px 14px",display:"flex",alignItems:"center",gap:6}}><span style={{fontSize:12,color:GL,fontWeight:600}}>{b.city}</span><span style={{fontSize:10,color:G,background:"#111",borderRadius:10,padding:"1px 7px"}}>{(b.days||"?")+" notti"}</span></div>
                      {bi<hotelBases.length-1&&<span style={{color:"#444",fontSize:16,margin:"0 2px"}}>{"\u2192"}</span>}
                    </React.Fragment>
                  ))}
                  {hotelLoad&&<div style={{display:"flex",alignItems:"center",gap:5,marginLeft:6}}><div className="dot"/><div className="dot"/><div className="dot"/><span style={{fontSize:11,color:G,marginLeft:2}}>ricerca in corso...</span></div>}
                </div>
              </div>
              ...
              {selNames.length>0&&<div style={{background:"#111",border:".5px solid "+G,borderRadius:10,padding:"12px 14px",marginBottom:"1rem",fontSize:12,color:"#ccc"}}><div style={{color:G,fontSize:11,letterSpacing:"1px",textTransform:"uppercase",marginBottom:6}}>{"\u2705"} Selezione</div>{selNames.map((n,ni)=><div key={ni}>{"\u2022 "+n}</div>)}</div>}
              ...
              {!showCust&&!showHotelNotes&&<div style={{display:"flex",gap:10,marginTop:8,flexWrap:"wrap"}}>
                ...
                {hotelBases.length>0&&<div style={{display:"flex",flexWrap:"wrap",gap:6,marginBottom:"0.8rem"}}>
                  {hotelBases.map((b,bi)=>(
                    <div key={bi} style={{background:"#1a1400",border:".5px solid "+G,borderRadius:20,padding:"5px 12px",display:"flex",alignItems:"center",gap:7}}>
                      ...
                      <button style={{background:"#111",border:".5px solid #444",borderRadius:"50%",width:18,height:18,color:"#aaa",fontSize:12,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}} onClick={e=>{e.stopPropagation();const nb=JSON.parse(JSON.stringify(hotelBases));nb[bi].days=Math.max(1,nb[bi].days-1);setHotelBases(nb)}}>&#8722;</button>
                      <span style={{fontSize:11,color:G,minWidth:18,textAlign:"center",fontWeight:600}}>{b.days}n</span>
                      <button style={{background:"#111",border:".5px solid #444",borderRadius:"50%",width:18,height:18,color:"#aaa",fontSize:12,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}} onClick={e=>{e.stopPropagation();const nb=JSON.parse(JSON.stringify(hotelBases));nb[bi].days=nb[bi].days+1;setHotelBases(nb)}}>+</button>
                      <button style={{background:"transparent",border:"none",color:"#555",fontSize:14,cursor:"pointer"}} onClick={e=>{e.stopPropagation();setHotelBases(hotelBases.filter((_,i)=>i!==bi))}}>{"\u2715"}</button>
                    </div>
                  ))}
                </div>}
              ...
              {!showCust&&!showHotelNotes&&selStr&&selKeys.length===0&&<Btn label={"\u2705 Scelgo "+selStr+" \u2192"} style={{marginTop:10}} onClick={()=>genDraft(selStr)}/>}
            </div>}
          </div>}
          ...
          {step===12&&<div className="card" style={{maxWidth:780}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,alignItems:"start"}}>
              ...
              <div style={{background:"#111",border:".5px solid #2a2a2a",borderRadius:12,padding:"14px 16px",maxHeight:360,overflowY:"auto"}}>
                ...
                {draftClusters.length===0?<div style={{fontSize:12,color:"#666"}}>{draftLoad?"Sto generando la bozza...":"La bozza non è ancora disponibile."}</div>
            ...
            <div className="rbody">
              ...
              {!finLoad&&finText&&<div>
                ...
                <div style={{background:"#1a1400",border:".5px solid #C9A84C",borderRadius:12,padding:"1.2rem 1.4rem",margin:"1.5rem 0 1rem"}}>
                  ...
                  <button onClick={saveItinerary} disabled={saving||saved} style={{background:saved?"#1a3a1a":G,border:"none",borderRadius:9,padding:"10px 24px",color:saved?"#6bff6b":DK,fontSize:13,fontWeight:600,cursor:saved?"default":"pointer",fontFamily:"'Inter',sans-serif",transition:"all .2s"}}>{saving?"⏳ Salvataggio...":saved?"✅ Salvato nel profilo":"💾 Salva itinerario"}</button>
                </div>
                ...
                <div style={{display:"flex",gap:12,flexWrap:"wrap",marginTop:"1.5rem",borderTop:".5px solid "+BRD,paddingTop:"1.2rem",alignItems:"center"}}>
                  ...
                  <button onClick={()=>navigate("/dashboard")} style={{background:"transparent",border:".5px solid #2a2a2a",borderRadius:10,padding:"10px 20px",color:"#888",fontSize:13,cursor:"pointer",fontFamily:"'Inter',sans-serif"}}>{"\uD83D\uDC64"} Vai al profilo</button>
                  <button onClick={()=>{try{window.scrollTo({top:0,behavior:"smooth"})}catch(e){window.scrollTo(0,0)}}} style={{background:"#1a1400",border:".5px solid "+G,borderRadius:10,padding:"10px 18px",color:G,fontSize:13,cursor:"pointer",fontFamily:"'Inter',sans-serif",fontWeight:500}}>{"\u2191"} Torna all'inizio</button>
